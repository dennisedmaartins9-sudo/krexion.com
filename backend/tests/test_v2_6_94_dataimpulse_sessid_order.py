"""DataImpulse username must be login__cr.us;st.ca;sessid.X — never login;sessid.X__cr.us;st.ca."""
...
def _assert_dataimpulse_ca_order(user: str) -> None:
    assert user.startswith(f"{_LOGIN}__cr.us;st.ca"), user
    assert user.find("__cr.us") < user.find(";st.ca") < user.find(";sessid."), user
    assert not user.startswith(f"{_LOGIN};sessid."), user
    assert "__cr.us;st.ca" in user
    # Session must not sit between login and country, or DataImpulse ignores st.ca.
...
def test_gateway_base_strips_mangled_live_username():
    mangled = f"{_LOGIN};sessid.55511122__cr.us;st.ca"
...
def test_rotate_dataimpulse_dot_sessid():
    before = f"{_LOGIN}__cr.us;st.ca;sessid.11111111"
    after = _rotate_session_in_username(before)
    assert after.startswith(f"{_LOGIN}__cr.us;st.ca;sessid.")
